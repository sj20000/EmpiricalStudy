# https://github.com/yongduosui/CAL/blob/main/model.py
...
import random
import pdb
from torch_geometric.nn import SAGEConv  #Class GraphSAGE from https://github.com/CODE-SUBMIT/Neighborhood-Enlargement-in-Graph-Network/blob/master/main.py
...
# https://github.com/CODE-SUBMIT/Neighborhood-Enlargement-in-Graph-Network/blob/master/main.py
'''
...
class GraphSAGE(torch.nn.Module):
    def __init__(self, num_features, 
        ...
        SConv = partial(SAGEConv, edge_norm=edge_norm, gfn=gfn)
        hidden_value = getattr(hidden, 'hidden')  
        hidden_value = 64 
        hidden_in = num_features
        self.bn_feat = BatchNorm1d(hidden_in)
       
        
        self.sage1 = SAGEConv(hidden_in, hidden_value)
        self.sage2 = SAGEConv(hidden_value, hidden_value)
        self.sage3 = SAGEConv(hidden_value, 64)
        self.bns_conv = torch.nn.ModuleList()
        self.convs = torch.nn.ModuleList()

        for i in range(num_conv_layers):
            self.bns_conv.append(BatchNorm1d(hidden_value))
            self.convs.append(SConv(hidden_value, hidden_value))
        self.bn_hidden = BatchNorm1d(hidden_value)
        self.bns_fc = torch.nn.ModuleList()
        self.lins = torch.nn.ModuleList()

        for i in range(num_fc_layers - 1):
            self.bns_fc.append(BatchNorm1d(hidden_value))
            self.lins.append(Linear(hidden_value, hidden_value))
        self.lin_class = Linear(hidden_value, num_classes)
...
class GCNNet_citation(torch.nn.Module):
    """GCN with BN and residual connection."""
    def __init__(self, num_features,
                        num_classes, hidden, 
                        num_feat_layers=1, 
                    ...
                    res_branch="BNConvReLU", global_pool="sum", dropout=0, 
                    edge_norm=True):
        super(GCNNet_citation, self).__init__()
...
class GATNet_citation(torch.nn.Module):
    def __init__(self, num_features, 
                       num_classes,
                       hidden,
                       head=4,
        ...
        super(GATNet_citation, self).__init__()
...
class GraphSAGE_citation(torch.nn.Module):
    def __init__(self, num_features, 
                    num_classes,
                    hidden,
                    num_feat_layers=1, 
                 ...
                 res_branch="BNConvReLU", global_pool="sum", dropout=0, 
                 edge_norm=True):
        super(GraphSAGE_citation,self).__init__()
...
class GINNet_citation(torch.nn.Module):
    def __init__(self, num_features,
                       num_classes,
                       hidden, 
                       num_fc_layers=2, 
        ...
        super(GINNet_citation, self).__init__()
        self.global_pool = global_add_pool
        self.dropout = dropout
        hidden_in = num_features
        hidden_out = num_classes
...
class CausalGCN_citation(torch.nn.Module):
    """GCN with BN and residual connection."""
    def __init__(self, num_features,
                       num_classes, args,
                       gfn=False, 
                       ...
                       dropout=0, 
                       edge_norm=True):
        super(CausalGCN_citation, self).__init__()
        num_conv_layers = args.layers
        hidden = args.hidden
        self.args = args
        self.global_pool = global_add_pool
...
class CausalGIN_citation(torch.nn.Module):
    """GCN with BN and residual connection."""
    def __init__(self, num_features,
                       num_classes, args,
                gfn=False,
                edge_norm=True):
        super(CausalGIN_citation, self).__init__()
    ...
    def forward(self, x, edge_index, eval_random=True, train_type="base"):
...
class CausalGAT_citation(torch.nn.Module):
    def __init__(self, num_features,
                       num_classes, 
                       args, 
                       head=4, 
                       dropout=0.2):
        super(CausalGAT_citation, self).__init__()
        num_conv_layers = args.layers
        hidden = args.hidden
        self.args = args
        self.global_pool = global_add_pool
